...
func FixtureDatabaseWithName(destroy_db types.AutodestroyDB, new_dbname types.Dbname, callback func(dbname types.Dbname)) {
...
func FixtureDatabase(destroy_db types.AutodestroyDB, callback func(dbname types.Dbname)) {
	new_dbname := types.Dbname(utils.TokenWord(8))
	FixtureDatabaseWithName(destroy_db, new_dbname, func(dbname types.Dbname) {
		callback(dbname)
	})
}

func FixtureConnTestDBWithName(destroy_db types.AutodestroyDB, new_dbname types.Dbname, callback func(dbname types.Dbname, conn *sql.DB, conn_orm *gorm.DB, bundb *bun.DB)) {
	FixtureDatabaseWithName(destroy_db, new_dbname, func(dbname types.Dbname) {
...
func FixtureConnTestDB(destroy_db types.AutodestroyDB, callback func(dbname types.Dbname, conn *sql.DB, conn_orm *gorm.DB, bundb *bun.DB)) {
	FixtureDatabase(destroy_db, func(dbname types.Dbname) {
		FixtureConn(dbname, func(dbname types.Dbname, conn *sql.DB, conn_orm *gorm.DB, bundb *bun.DB) {
			callback(dbname, conn, conn_orm, bundb)
		})
	})
