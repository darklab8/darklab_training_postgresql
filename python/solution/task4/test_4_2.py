...
def test_task4_2(database: Database, apply_task2_migrations: None, factories: TypeFactories) -> None:
    ...
    with database.get_core_session() as session:
        with measure_time(f"{amount_of_posts=}"):
            result = session.execute(query("query_4_2.sql", task, dict(N=selected, user_id=user.id)))
