...
def test_task7_3(database: Database, apply_task2_migrations: None, factories: TypeFactories, task_7_setup: tuple[int]) -> None:
    """
    ...
    with database.get_core_session() as session:
        with measure_time(f"{N=}"):
            result = session.execute(query("query_7_3.sql", task))
