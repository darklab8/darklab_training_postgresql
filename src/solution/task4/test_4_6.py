...
def test_task4_6(database: Database, apply_task2_migrations, factories: TypeFactories):
    ...
    target_posts = list(factories.post.create_batch([factories.post.template(author_id=user.id, tags=[target_tags[i % 50],random.choice(target_tags)]) for i in range(N)]))
    noise_posts = list(factories.post.create_batch([factories.post.template(author_id=user.id, tags=[random.choice(noise_tags),random.choice(noise_tags)]) for i in range(N)]))
    ...
    with database.get_core_session() as session:
        with measure_time(f"{N=}"):
            result = session.execute(query("query_4_6.sql", dict(N=selected), root=__file__))
