...
WITH data_per_day AS (
	...
	SELECT
		SUM(array_length(pel.tags,1)) OVER (
			PARTITION BY p.author_id, pel.created
		) as tag_sum_per_day,
		post_id, tags,created, author_id, status, rating
...
FROM user_ u
LEFT JOIN data_per_day first_year ON u.id = first_year.author_id AND first_year.created BETWEEN NOW() - interval '1 year' and NOW()
LEFT JOIN data_per_day second_year ON u.id = second_year.author_id AND second_year.created BETWEEN NOW() - interval '2 year' and NOW() - interval '1 year'
