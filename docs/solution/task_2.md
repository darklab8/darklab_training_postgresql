...
Написать скрипт для заполнения таблицы тестовыми данными. В базе данных должно быть не менее 100000 постов для разных пользователей; Для генерации тестовых данных можно воспользоваться функцией generate_series;

- Изначально я написал ответ на Python, но получилось не эффективено.
    - 10тысяч юзеров с 50 постами каждый создавались целых 14 секунд
    - при этом много десятков минут затрачивалось на выделения и удаления памяти
- Переписал на Golang с решением bottleneck ситуаций
    - за 11 секунд создается теперь 20 тысяч юзеров с 1'000'000 постов :)
    - затраты на выделения и удаления памяти включены в эти 11 секунд. 
    - Тесты ускорились в 200 раз.

Для этого я создаю в памяти Multi threaded(goroutines) workers
```go
--8<-- "golang/shared/worker.go"
```

Для которых создается generic Bulker отправляющий записи в постгрес
Bulker выделяет переиспользуемую память под ORM объекты объявления таблиц
И отправляет in bulk объекты в постгрес, на пределе допустимого колво Postgres параметров (65355) за один запрос
```go
--8<-- "golang/shared/bulk_create.go"
```

В свою очередь следующий testing fixture код определяет сколько объектов создать под какие таблицы
И задает связь между ними по Foreign ключам
```go
--8<-- "golang/fixture_fill_data.go"
```

Который в свою очередь используется для создания трех логических баз данных один раз для всех тестов, для оптимальности выполнения

```go
--8<-- "golang/testdb/testdb.go"
```

тестовые БД создаются через эту golang testing фичу выполняемую один раз для модуля

```go
--8<-- "golang/fixture_test.go"
```